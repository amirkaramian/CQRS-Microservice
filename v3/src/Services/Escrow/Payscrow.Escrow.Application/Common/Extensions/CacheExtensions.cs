...
namespace Payscrow.Escrow.Application
{
    ...
    public static class CacheExtensions
    {
        ...
        /// <param name="acquire">Function to load item if it's not in the cache yet</param>
        /// <returns>Cached item</returns>
        public static async Task<T> GetAsync<T>(this ICacheManager cacheManager, string key, int cacheTime, Func<Task<T>> acquire, bool ignoreCache = false)
        {
            if (!ignoreCache && cacheManager.IsSet(key))
            {
                return cacheManager.Get<T>(key);
            }
